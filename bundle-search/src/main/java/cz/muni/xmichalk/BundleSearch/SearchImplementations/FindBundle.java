...
public class FindBundle<T> implements ISearchBundle<T> {
    private final Function<String, Predicate<CpmDocument>> translatePredicate;
    private final Function<CpmDocument, T> resultTransformation;

    public FindBundle(Function<String, Predicate<CpmDocument>> getPredicate, Function<CpmDocument, T> resultTransformation) {
    ...
    public static Predicate<CpmDocument> translateMetaBundleIdToPredicate(String targetSpecification) {
        return (CpmDocument doc) -> {
            ...
            if (mainActivity != null) {
                ...
                if (value instanceof QualifiedName qnValue) {
                    return targetSpecification.equals(qnValue.getUri());
                }
            }
            return false;
        };
