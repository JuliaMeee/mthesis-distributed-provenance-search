...
def import_graph(
        document: provm.ProvDocument, json_data, token, meta_id, document_id, is_update=False,
):
    ...
    for bundle in document.bundles:  # not needed - only 1 bundle in document
        ...
        # logic not changed
        if is_update:
            (gen_entity, latest_entity, meta_bundle, new_version, token) = update_meta_prov(identifier, meta_id, token,
                                                                                            document_id)
            (activity, agent, new_agent, e, entity, meta_bundle) = store_token_into_meta(meta_bundle, new_version,
                                                                                         token)
            ...
            (first_version, gen_entity) = create_gen_entity_and_first_version(identifier)
            (activity, agent, new_agent, e, entity, meta_bundle) = store_token_into_meta(meta_bundle, first_version,
                                                                                         token)
...
# and then update his latest version from meta component (as in https://gitlab.ics.muni.cz/422328/dbprov)
def update_meta_prov(updated_bundle_id, meta_bundle_id, token, graph_id):
    ...
    entity_to_update = Entity.nodes.get(identifier=token['originatorId'] + "_" + graph_id)
    ...
    # Get the latest bundle entity and its version - # added in https://gitlab.ics.muni.cz/422328/dbprov TODO delete?
    versioned_entities = neomodel.Traversal(
        generator_entity,
        Entity.__label__,
        definition={
            "node_class": Entity,
    ...
    latest_entity = max(versioned_entities, key=lambda ent: ent.attributes[str(PAV_VERSION)])  # fix - str() added
    latest_version = latest_entity.attributes[str(PAV_VERSION)]

    # Create new updated entity
    new_version = Entity()
    new_version.identifier = f"{updated_bundle_id}"  # fix - was duplicated id of org
    new_version.attributes = dict(latest_entity.attributes)
    new_version.attributes[str(PAV_VERSION)] = latest_version + 1

    return generator_entity, latest_entity, meta_bundle, new_version, token
