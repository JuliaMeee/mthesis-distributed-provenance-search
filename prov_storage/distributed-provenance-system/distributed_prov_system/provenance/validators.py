...
from provenance.controller import IS_BACKBONE
from provenance.prov_doc_validators_strategies import ProvValidator
from provenance.constants import CPM_REFERENCED_META_BUNDLE_ID, CPM_REFERENCED_BUNDLE_ID, \
    CPM_HASH_ALG, CPM_REFERENCED_BUNDLE_HASH_VALUE,CPM_FORWARD_CONNECTOR, CPM_BACKWARD_CONNECTOR
from provenance.models import Document, Organization, Entity, Bundle, TrustedParty
...
def check_graph_id_belongs_to_meta(meta_provenance_id, graph_id, organization_id):
    ...
    # fix - was_revision_of relation instead of was_derived_from
    definition = dict(
        node_class=Entity,
    ...
    entity_traversal = neomodel.Traversal(entity, Entity.__label__, definition)
    ...
    if entity_traversal.all():
        raise DocumentError(
            f"Graph with given id [{graph_id}] is not the latest version."
    ...
    if url := connector.get_attribute(CPM_REFERENCED_BUNDLE_ID):
        ...
        if res2.status_code == 200:
            ...
            if "token" in json_data:
                hash1 = provm.first(connector.get_attribute(CPM_REFERENCED_BUNDLE_HASH_VALUE))
                hash_alg = provm.first(connector.get_attribute(CPM_HASH_ALG))
                token = json_data["token"]
                return resp, resp2, (token["data"]["documentDigest"] == hash1 and token["data"]["additionalData"][
                    "hashFunction"] == hash_alg)
...
def _check_connectors_references(futures):
    ...
    for future in concurrent.futures.as_completed(futures):
        ...
        if res is not None:  # fix - added check for None
            ...
            result.append((filter_own_address(parsed_url), connector)) # used before for connectors - may be used in future
    return True, result
...
class InputGraphChecker:
    ...
    def validate_graph(self):
        ...
        are_ok_backward_connectors = CPM_VALIDATOR.check_backward_connectors_attributes(self._backward_connectors)
        if not are_ok_backward_connectors:
            raise DocumentError(
                f"Backward connector(s) is/are missing mandatory attributes."
            )

        are_ok_forward_connectors = CPM_VALIDATOR.check_forward_connectors_attributes(self._forward_connectors)
        ...
        is_cpm_ok, error_message = CPM_VALIDATOR.check_cpm_constraints(self._prov_bundle, self._forward_connectors,
                                                                       self._backward_connectors, self._main_activity)
        if not is_cpm_ok:
            raise DocumentError(
                f"CPM problem: {error_message}"
            )
    ...
    def _retrieve_connectors_from_graph(self):
        ...
        for entity in self._prov_bundle.get_records(provm.ProvEntity):
            ...
            if IS_BACKBONE.is_backbone_element(entity,
                                               self._prov_bundle):  # change - check whether connector belongs to traversal information
                for t in prov_types:
    ...
    def _check_namespaces(self):
        ...
        for element in self._prov_bundle.get_records(class_or_type_or_tuple = (ProvAgent, ProvActivity, ProvEntity)):
