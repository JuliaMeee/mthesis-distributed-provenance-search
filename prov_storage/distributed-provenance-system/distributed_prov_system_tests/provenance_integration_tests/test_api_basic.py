...
class MyTestCase(TestCase):
    ...
    def test_010register_org_1_ok(self):
        res = register_org1_to_hospital_storage(self.prov_data)
    ...
    def test_090store_graph_with_backbone_with_backward_connector_ok(
        ...
        res = register_org_2_to_pathology_storage(self.prov_data)
        self.assertEqual(201, res.status_code)
    ...
    def test_10retrieve_meta_basic(self):
        ...
        specialziation_gen_concrete_bundle = list(meta_bundle_returned.get_records(ProvSpecialization))[0]
        self.assertEqual(first_doc._identifier,
                         next(iter(specialziation_gen_concrete_bundle.get_attribute(PROV_ATTR_SPECIFIC_ENTITY))))
        self.assertEqual(gen_doc._identifier,
                         next(iter(specialziation_gen_concrete_bundle.get_attribute(PROV_ATTR_GENERAL_ENTITY))))
        ...
        self.assertEqual(str(CPM_TOKEN), list(filter(lambda x: x[0] ==
                                                               meta_bundle_returned.valid_qualified_name(PROV_TYPE),
                                                     token_attributes))[0][1])
    ...
    def test_10retrieve_meta_basic_rdf(self):
        ...
        # check relations between first doc and token
        for e in meta_bundle_returned.records:
            print(e)
        bundle_token, trusted_party = self.check_meta_concrete_component_and_token(first_doc, meta_bundle_returned)
    ...
    def test_112_retrieve_meta_with_2_versions(self):
        ...
        specialization_gen_concrete_bundle_2 = [x for x in specializations if
                                                first(x.get_attribute(PROV_ATTR_SPECIFIC_ENTITY)) ==
        ...
        self.assertEqual({"prov:Revision"},
        ...
        # check type of agent - trusted party
        self.assertEqual(str(CPM_TRUSTED_PARTY), list(
            filter(lambda x: x[0] == meta_bundle_returned.valid_qualified_name(PROV_TYPE), tp_attributes))[0][1])
        ...
        self.assertEqual(str(CPM_TOKEN), list(
            filter(lambda x: x[0] == meta_bundle_returned.valid_qualified_name(PROV_TYPE), token2_attributes))[0][1])
    ...
    def test_1130_retrieve_meta_bundle_multiple_general_docs_in_one_meta_bundle(self):
        ...
        specialization_gen_concrete_bundle = [x for x in specializations if
                                              ...
                                              first_doc._identifier]
        self.assertEqual(1, len(specialization_gen_concrete_bundle))
        self.assertEqual(gen_doc_2._identifier,
                         next(iter(specialization_gen_concrete_bundle[0].get_attribute(PROV_ATTR_GENERAL_ENTITY))))

        specialization_gen_concrete_bundle_2 = [x for x in specializations if
                                                first(x.get_attribute(PROV_ATTR_SPECIFIC_ENTITY)) ==
                                                second_doc._identifier]
        self.assertEqual(1, len(specialization_gen_concrete_bundle_2))
        self.assertEqual(gen_doc_2._identifier,
                         next(iter(specialization_gen_concrete_bundle_2[0].get_attribute(PROV_ATTR_GENERAL_ENTITY))))
    ...
    def test_1131_retrieve_meta_bundle_multiple_general_docs_with_multiple_versions_in_one_meta_bundle(self):
        ...
        doc2, bundle, meta_bundle_id, backbone_basic, prev_cpm_bundle_info, _ = helpers.create_cpm_provenance_basic(
            self.prov_data.existential_variable_prefix,
            storage_name=provenance_storage_hospital_name,
            org_name=self.prov_data.org_id,
            main_activity_attributes=self.prov_data.main_activity_attributes_ok,
        ...
        # different suffix, update first bundle - according to request url
        res = requests.put(
        ...
        specialization_gen_concrete_bundle_3 = [x for x in specializations if
                                                first(x.get_attribute(PROV_ATTR_SPECIFIC_ENTITY)) ==
                                                new_doc_2._identifier]
        self.assertEqual(1, len(specialization_gen_concrete_bundle_3))
        self.assertEqual(gen_doc._identifier,
                         next(iter(specialization_gen_concrete_bundle_3[0].get_attribute(PROV_ATTR_GENERAL_ENTITY))))

        specialization_gen_concrete_bundle_2 = [x for x in specializations if
                                                first(x.get_attribute(PROV_ATTR_SPECIFIC_ENTITY)) ==
                                                third_doc._identifier]
        self.assertEqual(1, len(specialization_gen_concrete_bundle_2))
        self.assertEqual(gen_doc_2._identifier,
                         next(iter(specialization_gen_concrete_bundle_2[0].get_attribute(PROV_ATTR_GENERAL_ENTITY))))
        ...
        self.assertEqual(second_doc._identifier,
                         next(iter(derivation[0].get_attribute(PROV_ATTR_GENERATED_ENTITY))))

        derivation = [x for x in derivations if
                      first(x.get_attribute(PROV_ATTR_USED_ENTITY)) ==
                      new_doc._identifier]
        self.assertEqual(1, len(derivation))
        self.assertEqual(new_doc_2._identifier,
                         next(iter(derivation[0].get_attribute(PROV_ATTR_GENERATED_ENTITY))))
        ...
        derivation = [x for x in derivations if
                      ...
                      second_doc._identifier]
        self.assertEqual(1, len(derivation))
        self.assertEqual(third_doc._identifier,
                         next(iter(derivation[0].get_attribute(PROV_ATTR_GENERATED_ENTITY))))
    ...
    def check_meta_concrete_component_and_token(self, first_doc, meta_bundle_returned, num_verison=0, id_suffix=""):
        token_generation = \
        ...
        self.assertEqual(token_generation.identifier,
                         first(usage_records[num_verison].get_attribute(PROV_ATTR_ACTIVITY)))
        self.assertEqual(first_doc.identifier,
                         first(usage_records[num_verison].get_attribute(PROV_ATTR_ENTITY)))
        ...
        self.assertEqual(trusted_party.identifier,
                         first(association_records[num_verison].get_attribute(PROV_ATTR_AGENT)))
        self.assertEqual(token_generation.identifier,
                         first(association_records[num_verison].get_attribute(PROV_ATTR_ACTIVITY)))
        bundle_token = meta_bundle_returned.get_record(
            f"{self.prov_data.org_id}_test_{self.prov_data.timestamp}_bundle{id_suffix}_token")[
            0]
        generation_records = list(meta_bundle_returned.get_records(ProvGeneration))
        self.assertEqual(token_generation.identifier,
                         first(generation_records[num_verison].get_attribute(PROV_ATTR_ACTIVITY)))
        self.assertEqual(bundle_token.identifier,
                         first(generation_records[num_verison].get_attribute(PROV_ATTR_ENTITY)))
        attribution_tokens = list(meta_bundle_returned.get_records(ProvAttribution))
        self.assertEqual(trusted_party.identifier,
                         first(attribution_tokens[num_verison].get_attribute(PROV_ATTR_AGENT)))
        self.assertEqual(bundle_token.identifier,
                         first(attribution_tokens[num_verison].get_attribute(PROV_ATTR_ENTITY)))
