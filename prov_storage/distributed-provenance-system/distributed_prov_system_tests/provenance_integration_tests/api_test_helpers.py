...
import helpers
from constants import CPM_MAIN_ACTIVITY, CPM_REFERENCED_META_BUNDLE_ID, CPM_BACKWARD_CONNECTOR, \
...
def register_org1_to_hospital_storage(prov_data):
    ...
    res = requests.post(
        "http://" + provenance_storage_hospital_url + "/api/v1/organizations/" + prov_data.org_id,
        json.dumps(json_data))
    return res


def register_org_2_to_pathology_storage(prov_data):
    with open('int1.pem', 'r') as file:
        intermediate_cert_1 = file.read()
    with open('int2.pem', 'r') as file:
        intermediate_cert_2 = file.read()
    ...
    with open("./test_cert2.pem", "w") as file:
        file.seek(0)
        file.write(parse_certificate(client_cert, as_string=True))
    with open("./test_sign2.pem", "w") as file:
        file.seek(0)
        file.write(key.private_bytes(Encoding.PEM, PrivateFormat.PKCS8, NoEncryption()).decode())
    client_cert = parse_certificate(client_cert, as_string=True)
    json_data = {}
    ...
    json_data["intermediateCertificates"] = [intermediate_cert_1, intermediate_cert_2]
    res = requests.post(
        "http://" + provenance_storage_pathology_url + "/api/v1/organizations/" + prov_data.org_id2,
        json.dumps(json_data))
    return res
...
class TestDataCreator(ABC):
    ...
    doc, bundle, meta_bundle_id, backbone_basic, first_cpm_bundle_info, _ = helpers.create_cpm_provenance_basic(
    ...
    doc2, backbone_with_backwards_connector, next_meta_bundle_id, next_cpm_bundle_info = helpers.create_cpm_provenance_with_backward_connector(
    ...
    doc_fw, bundle_fw, meta_bundle_id_w_forward, backbone_basic_w_forward, cpm_bundle_info_forward_conn_doc = helpers.create_cpm_with_forward_connector(
        existential_variable_prefix,
        storage_name=provenance_storage_hospital_name,
    ...
    doc_updated, bundle_updated, meta_bundle_id_updated, backbone_basic_updated, _, _ = helpers.create_cpm_provenance_basic(
        existential_variable_prefix,
        storage_name=provenance_storage_hospital_name,
        org_name=org_id,
        main_activity_attributes=main_activity_attributes_ok,
    ...
    doc_end, bundle_end, meta_bundle_id_end, backbone_basic_end, last_cpm_bundle_info = helpers.create_cpm_provenance_basic_without_fw_connector(
        existential_variable_prefix,
        storage_name=provenance_storage_hospital_name,
        org_name=org_id,
    ...
    # call sub-function with prepared attributes
    doc_with_both_connectors, backbone_with_both_connectors, middle_meta_bundle_id, middle_cpm_bundle_info = helpers.create_cpm_provenance_with_backward_connector(
    ...
    # prepare component - provenance chain end with added backward connectors referencing previous documents
    ...
    document_with_backwards_connector_updated, bundle_with_backwards_connector_updated, meta_bundle_id_end, backbone_doc_with_backwards_connector_updated, last_cpm_bundle_info_3 = (
