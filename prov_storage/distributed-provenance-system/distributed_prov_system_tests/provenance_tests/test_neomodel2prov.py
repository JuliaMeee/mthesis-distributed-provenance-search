...
class MyTestCase(TestCase):
    ...
    def test_convert_meta_to_prov(self, MockTraversal, MockAddVersionChainToBundle):
        neo_bundle = Bundle()
        ...
        self.assertEqual(list(document.namespaces)[0], Namespace("meta", DEFAULT_NAMESPACE + f"/api/v1/documents/meta/"))
    ...
    def test_convert_tuple_list_to_dict(self):
    ...
    def side_effect_entity_used_all(self, *args, **kwargs):
        ...
        token = Entity(identifier = f"ORG_entity_id_{ver}_token") # checked - fix in neomodel2prov - now creating qualified_name from this id
    ...
    @patch('provenance.neomodel2prov.neomodel.Traversal.all')
    def test_add_version_chain_to_bundle(self, MockTraversal):
        ...
        MockTraversal.return_value = generated_entities  #called get_sorted_specialized_entities_from_gen - mock traversal
        ...
        for x in range(5):
            ...
            self.assertEqual(list(convert_attributes_to_dict(generated_entities[x].used.all()[0].was_generated_by.all()[0]).items())[0],
        ...
        for x in range(5):
            ...
            self.assertEqual(
                list(convert_attributes_to_dict(generated_entities[x].used.all()[0].was_generated_by.all()[0]).items())[0],
        ...
        for x in range(4):
            self.assertEqual((QualifiedName(NAMESPACES["prov"], "type"), "prov:Revision"),
    ...
    @patch('provenance.neomodel2prov.neomodel.Traversal.all')
    def test_add_version_chain_to_bundle_agent_in_bundle(self, MockTraversal):
        bundle = ProvBundle(identifier=QualifiedName(NAMESPACES["meta"], "prov"))
        bundle.set_default_namespace(DEFAULT_NAMESPACE)
        gen_entity = Entity()
        gen_entity.identifier = f"ORG_entity_gen"  # called get_entity_qualified_name
