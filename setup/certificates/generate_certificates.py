...
def export_cert(cert, filepath):
    with filepath.open("wb") as file:
        file.write(cert.public_bytes(serialization.Encoding.PEM))
        
def main(): 
    org1_key, org1_cert = generate_certificate("SK", "ORG1", auth_key=Path("int2.key"),
                                            auth_cert=Path("int2.pem"),
                                            ca=False,
                                            path_length=None,
                                            )
    
    export_key(org1_key, Path("./ORG1.key"))
    export_cert(org1_cert, Path("./ORG1.pem"))

    org2_key, org2_cert = generate_certificate("SK", "ORG2", auth_key=Path("int2.key"),
                                            auth_cert=Path("int2.pem"),
                                            ca=False,
                                            path_length=None,
                                            )
    
    export_key(org2_key, Path("./ORG2.key"))
    export_cert(org2_cert, Path("./ORG2.pem"))
    
    org3_key, org3_cert = generate_certificate("SK", "ORG3", auth_key=Path("int2.key"),
                                            auth_cert=Path("int2.pem"),
                                            ca=False,
                                            path_length=None,
                                            )
