...
class MyTestCase(TestCase):
    ...
    def test_130_store_graph_with_backbone_with_concrete_forward_connector_ok(
            self):

        json_data = create_json_for_doc_storing(self.prov_data.doc_fw)

        res = requests.put(
            # put - im updating the basic document according to backward connector form second doc - adding concrete forward connector
    ...
    def test_1392_store_graph_with_backbone_end_ok_update_add_backward_connectors(
        ...
        self.assertEqual(doc_returned, jcs.canonicalize(
            provjson.encode_json_document(self.prov_data.document_with_backwards_connector_updated)))
    ...
    def test_1393_store_graph_with_backbone_first_ok_update_add_two_forward_connectors(
        ...
        self.assertEqual(doc_returned,
                         jcs.canonicalize(provjson.encode_json_document(self.prov_data.doc_two_forward_conns)))
    ...
    def check_token(self, json_data, token, org_id, storage_name, bundle_suffix=""):
        """Checks the fields of returned token

        :param json_data: data sent in request to store a component
        :param token: token returned by provenance storage service
    ...
    def check_signature(self, token):
        """Checks the signature of component created by trusted party
