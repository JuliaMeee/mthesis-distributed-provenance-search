...
class MyTestCase(unittest.TestCase):
    def test_create_gen_entity_and_first_version_returns_first_version_and_general_entity(self):
    ...
    def test_update_meta_prov_creates_and_returns_new_version(self, MockEntityNodes, MockBundle):
        graph_id = "id1"
        ...
        mock_first.specialization_of.all = MagicMock()
        mock_first.specialization_of.all.return_value = gen_entity
        meta_bundle.contains.filter = MagicMock(return_value=mock_first)
        MockBundle.nodes.get = MagicMock(return_value=meta_bundle)
        first_entity.specialization_of.all = MagicMock()
        first_entity.specialization_of.all.return_value = iter([gen_entity])
        MockEntityNodes.get = MagicMock(return_value=first_entity)

        (gen_entity, latest_entity2, meta_bundle_1, new_version, token2) = update_meta_prov(new_entity_id, meta_id,
                                                                                            token, graph_id)
        ...
        self.assertEqual(2, new_version.attributes[str(PAV_VERSION)])
        self.assertEqual("id_gen", gen_entity.identifier)
        self.assertEqual(first_entity, latest_entity2)
        self.assertEqual(token, token2)
        MockEntityNodes.get.assert_called_with(identifier=token['originatorId'] + "_" + graph_id)

    @patch('provenance.prov2neomodel.get_tp_agent')
    def test_store_token_into_meta_creates_and_returns_activity_and_token(self, getAgent):
        meta_bundle = Bundle()
        meta_bundle.identifier = "id"
        entity = Entity()
        ...
        (activity, agent_returned, new_agent, e, entity2, meta_bundle2) = store_token_into_meta(meta_bundle, entity,
                                                                                                token)
    ...
    def test_get_TP_agent_no_agent_creates_new_agent(self, MockTraversal):
        meta_bundle = Bundle()
        meta_bundle.identifier = "id"
        token = get_dummy_token()["data"]
        ...
        self.assertEqual(token['additionalData']['trustedPartyCertificate'],
                         agent2.attributes[str(CPM_TRUSTED_PARTY_CERTIFICATE)])

    @patch('provenance.prov2neomodel.neomodel.Traversal.all')
    def test_get_TP_agent_agent_exists_returns_old_agent(self, MockTraversal):
        meta_bundle = Bundle()
        meta_bundle.identifier = "id"
        token = get_dummy_token()["data"]
        agent = Agent()
    ...
    def test_save_new_agent_saves_agent(self):
        agent = MagicMock()
    ...
    def test_save_version_to_db_saves_records_and_creates_relations(self):
    ...
    def test_update_meta_prov_save_saves_records_and_creates_relations(self):
        gen_entity = Entity()
    ...
    def test_store_token_into_meta_save_saves_records_and_creates_relations(self):
        activity = Activity()
    ...
    def test_import_graph_no_bundle_throws_exception(self):
    ...
    def test_import_graph_many_bundles_throws_exception(self):
        document = provm.ProvDocument()
        json_data = {}
        token = get_dummy_token()
        graph_id = "id_graph"
    ...
    def test_import_graph_new_bundle_not_update_imports_component(self, MockDoesnotExist, MockBundle, MockDocument,
                                                                  MockTraversal, MockDb, MockSaveVersion, MockSaveAgent,
                                                                  MockStoreToken):
        document = provm.ProvDocument()
    ...
    @patch('provenance.prov2neomodel.Document', autospec=True)
    @patch('provenance.prov2neomodel.Bundle')
    def test_import_graph_old_bundle_not_update_imports_component(self, MockBundle, MockDocument, MockTraversal, MockDb,
    ...
    @patch('provenance.prov2neomodel.Bundle')
    @patch('provenance.prov2neomodel.Entity.nodes')
    def test_import_graph_update_imports_component(self, MockEntityNodes, MockBundle, MockDocument, MockTraversal,
                                                   MockDb,
        ...
        last_entity.specialization_of.all = MagicMock()
        last_entity.specialization_of.all.return_value = iter([gen_entity])
        MockEntityNodes.get = MagicMock()
        MockEntityNodes.get.return_value = last_entity
