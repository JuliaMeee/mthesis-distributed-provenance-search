...
from prov.constants import PROV_ATTR_SPECIFIC_ENTITY, PROV_ATTR_GENERAL_ENTITY, PROV_TYPE, PROV_ATTR_STARTTIME, \
    PROV_ATTR_ENDTIME, PROV_ATTR_INFLUENCEE, PROV_ATTR_INFLUENCER, PROV_ATTR_INFORMED, PROV_ATTR_INFORMANT, \
    PROV_ATTR_ALTERNATE1, PROV_ATTR_ALTERNATE2, PROV_ATTR_ENTITY, PROV_ATTR_ACTIVITY, PROV_ATTR_STARTER, \
    PROV_ATTR_ENDER, PROV_ATTR_GENERATED_ENTITY, PROV_ATTR_USED_ENTITY, PROV_ATTR_AGENT, PROV_ATTR_DELEGATE, \
    PROV_ATTR_RESPONSIBLE, PROV_ATTR_COLLECTION, PROV_ATTR_TRIGGER

from provenance.constants import CPM_BACKWARD_CONNECTOR, CPM_FORWARD_CONNECTOR, CPM_MAIN_ACTIVITY, CPM_RECEIVER_AGENT, \
    CPM_SENDER_AGENT, CPM_ID, CPM, DCT, DCT_HAS_PART
...
def get_prov_generations_usages_attributions_agents(bundle):
    ...
    return generations, usages, attributions, sender_agents, receiver_agents, specializations, derivations, all_relations
...
def get_backbone_and_domain(graph, graph_format, is_backbone_strategy):
    ...
    for record in records:
        if isinstance(record, provm.ProvActivity) or isinstance(record, provm.ProvEntity) or isinstance(record,
                                                                                                        provm.ProvAgent):
...
def relation_belongs_to_bb(records_bb_ids, relation):
    ...
    if isinstance(relation, provm.ProvInfluence):
        if next(iter(relation.get_attribute(PROV_ATTR_INFLUENCEE))) in records_bb_ids and next(
                iter(relation.get_attribute(PROV_ATTR_INFLUENCER))) in records_bb_ids:
            return True
    if isinstance(relation, provm.ProvCommunication):
        if next(iter(relation.get_attribute(PROV_ATTR_INFORMED))) in records_bb_ids and next(
                iter(relation.get_attribute(PROV_ATTR_INFORMANT))) in records_bb_ids:
            return True
    if isinstance(relation, provm.ProvAlternate):
        if next(iter(relation.get_attribute(PROV_ATTR_ALTERNATE1))) in records_bb_ids and next(
                iter(relation.get_attribute(PROV_ATTR_ALTERNATE2))) in records_bb_ids:
            return True
    if isinstance(relation, provm.ProvGeneration):
        if next(iter(relation.get_attribute(PROV_ATTR_ENTITY))) in records_bb_ids and next(
                iter(relation.get_attribute(PROV_ATTR_ACTIVITY))) in records_bb_ids:
            return True
    if isinstance(relation, provm.ProvUsage):
        if next(iter(relation.get_attribute(PROV_ATTR_ACTIVITY))) in records_bb_ids and next(
                iter(relation.get_attribute(PROV_ATTR_ENTITY))) in records_bb_ids:
            return True
    ...
    if isinstance(relation, provm.ProvDerivation):
        if next(iter(relation.get_attribute(PROV_ATTR_GENERATED_ENTITY))) in records_bb_ids and next(
                iter(relation.get_attribute(PROV_ATTR_USED_ENTITY))) in records_bb_ids:
            return True
    if isinstance(relation, provm.ProvAttribution):
        if next(iter(relation.get_attribute(PROV_ATTR_ENTITY))) in records_bb_ids and next(
                iter(relation.get_attribute(PROV_ATTR_AGENT))) in records_bb_ids:
            return True
    ...
    if isinstance(relation, provm.ProvDelegation):
        if next(iter(relation.get_attribute(PROV_ATTR_DELEGATE))) in records_bb_ids and next(
                iter(relation.get_attribute(PROV_ATTR_RESPONSIBLE))) in records_bb_ids:
            return True
    if isinstance(relation, provm.ProvSpecialization):
        if next(iter(relation.get_attribute(PROV_ATTR_SPECIFIC_ENTITY))) in records_bb_ids and next(
                iter(relation.get_attribute(PROV_ATTR_GENERAL_ENTITY))) in records_bb_ids:
            return True
    ...
    if isinstance(relation, provm.ProvMembership):
        if next(iter(relation.get_attribute(PROV_ATTR_COLLECTION))) in records_bb_ids and next(
                iter(relation.get_attribute(PROV_ATTR_ENTITY))) in records_bb_ids:
            return True
    return False
