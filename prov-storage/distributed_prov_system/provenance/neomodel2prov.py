...
def add_version_chain_to_bundle(bundle: ProvBundle, gen_entity: Entity):
    ...
    def get_entity_qualified_name(entity: Entity):
        ...
        try:
            ...
            ns = Namespace(
                org,
                "http://" + config.fqdn + f"/api/v1/organizations/{org}/documents/",
            )
            NAMESPACES[org] = ns
    ...
    def add_signing_activity_to_bundle(
        ...
        neo_activity = list(neo_entity.used.all())[0]  # signing activity is first and of type used
    ...
    activity = add_signing_activity_to_bundle(first_entity, tp, prov_representation_first_entity)
    ...
    while derived_entities:
        database_representation_prev_entity, prov_representation_prev_entity = derived_entities.pop()
        traversal = neomodel.Traversal(database_representation_prev_entity, Entity.__label__, definition_revisions)
        ...
        for curr_neo_entity in current_derived_entities:
            ...
            activity = add_signing_activity_to_bundle(curr_neo_entity, tp, prov_representation_curr_derived_entity)
            add_token_to_bundle(curr_neo_entity, activity, tp)

            derived_entities.append((curr_neo_entity, prov_representation_curr_derived_entity))
